...
color <- brewer.pal(n = 5, name = "Set2")[c(1:5)]
...
p <- ggplot(F, aes(RDA1, RDA2)) +
  geom_point(aes(color = label$cols), size=2.5) +
  ...
  geom_segment(data = rda_tb_forward_r.env, aes(x = 0, y = 0, xend = RDA1,yend = RDA2), arrow = arrow(length = unit(0.2, 'cm')), size = 0.5, color = 'brown') +
  geom_text(data = rda_tb_forward_r.env, aes(RDA1 * 1.1, RDA2 * 1.1, label = sample), color = 'brown', size = 4)+
...
#分类样本提取----
taxa <- read.table("select_taxa2.txt",header=T,stringsAsFactors = F)
...
#选择TAXA_new-----
taxa_north <- taxa_EA_N[sort(round(runif(25,1,length(taxa_EA_N))))]
taxa_south <- taxa_EA_N[sort(round(runif(25,1,length(taxa_EA_S))))]
taxa_WA_25 <- taxa_EA_N[sort(round(runif(25,1,length(taxa_WA))))]
taxa_EU_25 <- taxa_EA_N[sort(round(runif(25,1,length(taxa_EU))))]
taxa_SCA_25 <- taxa_EA_N[sort(round(runif(25,1,length(taxa_SCA))))]
#筛选样本变异----
...
#RDA分析(north_25)----
#north
north_temp_rda <- rda(phylum_north~., temp_EA_N, scale = FALSE)
RsquareAdj(north_temp_rda)
north_prec_rda <- rda(phylum_north~., prec_EA_N, scale = FALSE)
RsquareAdj(north_prec_rda)
north_all_rda <- rda(phylum_north~., env_EA_N, scale = FALSE)
RsquareAdj(north_all_rda)
#RDA分析(south_25)----
#South
South_temp_rda <- rda(phylum_south~., temp_EA_S, scale = FALSE)
RsquareAdj(South_temp_rda)
South_prec_rda <- rda(phylum_south~., prec_EA_S, scale = FALSE)
