...
for i in `cat /data1/home/yafei/008_Software/snpEff/Xp-clr_6VIP/names.txt`
do
...
library(ggplot2)
library(ggseqlogo)
setwd("/Users/guoyafei/Documents/01_Migration/02_Environment/02_XP-CLR/Gene/V5/Plot")
substrRight <- function(x){
  num = nchar(x)-2
  gsub('[0-9.]', '', substr(x, 6, nchar(x)))
}
names <- read.table("/Users/guoyafei/Documents/01_Migration/02_Environment/02_XP-CLR/Gene/V5/names.txt",header=F,stringsAsFactors = F,sep="\t")
names <- names[,1]
#names <- c("","","","","","","","","","","","","","","","","GA2ox3-A1","","GA2ox3-D1","","","","","","","","","","","","","","","","","","","","","","","","","","","","NGR5_1","NGR5_2","NGR5_3","PIF_1","PIF_2","PIF_3")

pdf("snpEff.pdf", width = 12, height = 4)
for (i in c(1:length(names))){
  ...
  fasta = fasta_file[,2]
  file2 <- paste("/Users/guoyafei/Documents/01_Migration/02_Environment/02_XP-CLR/Gene/V5/snpEff/",names[i],".snpEff",sep="")
  ...
  p3 <- ggplot(bp_data, aes(factor(x), Impact))+
    ...
    theme_logo()+
    xlab("")+
    theme(axis.text.x = element_text(angle = 90,size=15),axis.text.y = element_blank())
